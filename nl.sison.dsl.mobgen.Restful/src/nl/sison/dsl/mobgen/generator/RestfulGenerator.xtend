...
class RestfulGenerator implements IGenerator {
	
	override void doGenerate(Resource resource, IFileSystemAccess fsa) {
		new AndroidHttpRequestGenerator().doGenerate(resource, fsa)
	}
	...
	def createCompositeJsonValue(JsonCompositeValue composite, StringBuilder parserString, int recursionDepth)
	{
		if (composite.objectValue != null) // TODO check for correctness
		{
			...
			for (keyValue : composite.objectValue.keyValuePair)
			{
				...
				if (scalar != null)
				{
					if (scalar.metaScalarValue == null)
					{
						val putStr = '''
						n«recursionDepth».put(«keyValue.key», «scalar.determineChildType»);
...
//			''')
	...
	def CharSequence determineChildType(JsonLiteralValue value)
	{
		switch (value)
		{
			case value.literalString : return value.toString
			case value.emptyObject : return 'new JsonObject()'
			case value.emptyArray : return 'new JsonArray()'
			case value.literalNull : return 'JSONObject.NULL'
			case value.numberFloat : return value.toString
			case value.numberInteger : return value.toString
//			case value.booleanValue : return value. // TODO figure out how to read literal boolean value
//			case value.enumType // TODO
//			case value.metaScalarValue : return value.me // TODO
		}
	}
...
*/
	 ...
	 * 
	 * TODO Add functionality for cached results from calls
	 * TODO Add functionality for deferred calls
	 */
