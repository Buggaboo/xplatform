...
public class AndroidHttpRequestGenerator implements IGenerator {
      ...
      for (final JsonKeyValuePair keyValue : _keyValuePair) {
        {
          ...
          boolean _notEquals_1 = (!Objects.equal(scalar, null));
          if (_notEquals_1) {
            JsonMetaScalarType _metaScalarValue = scalar.getMetaScalarValue();
            boolean _equals = Objects.equal(_metaScalarValue, null);
            if (_equals) {
              StringConcatenation _builder_1 = new StringConcatenation();
              _builder_1.append("n");
              ...
              _builder_1.append(", ");
              CharSequence _determineChildType = this.determineChildType(scalar);
              _builder_1.append(_determineChildType, "");
              _builder_1.append(");");
              _builder_1.newLineIfNotEmpty();
  ...
  public CharSequence determineChildType(final JsonLiteralValue value) {
    boolean _matched = false;
    if (!_matched) {
      boolean _isLiteralString = value.isLiteralString();
      if (_isLiteralString) {
        _matched=true;
    ...
    if (!_matched) {
      boolean _isEmptyObject = value.isEmptyObject();
      if (_isEmptyObject) {
        _matched=true;
    ...
    if (!_matched) {
      boolean _isEmptyArray = value.isEmptyArray();
      if (_isEmptyArray) {
        _matched=true;
    ...
    if (!_matched) {
      boolean _isLiteralNull = value.isLiteralNull();
      if (_isLiteralNull) {
        _matched=true;
    ...
    if (!_matched) {
      boolean _isNumberFloat = value.isNumberFloat();
      if (_isNumberFloat) {
        _matched=true;
        return value.toString();
      }
    ...
    if (!_matched) {
      boolean _isNumberInteger = value.isNumberInteger();
      if (_isNumberInteger) {
        _matched=true;
        return value.toString();
      }
